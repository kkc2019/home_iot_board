...
void setup() {
  ...
  Serial.println("u = angle + 90");   // u를 누른다면 현재 각도값에서 +15도
...
void loop() {
  if(Serial.available())  // 시리얼모니터가 사용가능할 때
  {
    ...
    {
      Serial.print("+90");  // '+90'를 시리얼 모니터에 출력
      for(int i = 0; i < 90; i++)  // 현재 각도에서 90도 더해주기
      {
        angle = angle + 1;   
        if(angle >= 180)
          angle = 180;
    ...
    {
      Serial.print("\t-90\t");  // '-90'라고 출력
      for(int i = 0 ; i < 90 ; i++)  // 현재 각도에서 90도 빼주기
      {
        angle = angle - 1;
        if(angle <= 0)
          angle = 0;
